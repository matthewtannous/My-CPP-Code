/*
std::unordered_multimap is an unsorted associative container that contains
...
std::ostream& operator<<(std::ostream& out,const std::unordered_multimap<KeyType, ValueType>& map)
{
    cout << "\n\nSize: " << map.size() << "\n";
...
void test1()
{
    ...
    std::unordered_multimap<string, int> people = {{"Mat", 20}, {"Tan", -10},
                {"George G",3}, {"Tan", 41}};

    // Access one element with key
    std::unordered_multimap<string, int>::iterator iter = people.find("Tan");
    if (iter == people.end())
    {
        cout << "Not found";
    }
    ...
    std::unordered_multimap<string, int>::iterator iter2 = people.find("Tan");
    if (iter2 == people.end())
    {
        cout << "Not found";
    }
    ...
    std::unordered_multimap<string, int> people_normal_order = {{"Mat", 20}, {"Tan", -10},
                {"George G",3}};
    // std::unordered_multimap<string, int, std::greater<string>> people_reverse_order = {{"Mat", 20}, {"Tan", -10},
    //             {"George G",3}};
...
void test2()
{
    std::unordered_multimap<string, Person> map{{"joe", Person()}, {"mat", Person("Matthew Tannous", 19)}};
    cout << map;
    ...
    std::unordered_multimap<string, Person>::iterator it2 = map.find("mat");
    if (it2 == map.end())
    {
        cout << "Not found"
